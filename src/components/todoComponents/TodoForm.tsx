...
function TodoForm() {
  ...
  return (
    <div className="py-4">
      <form className="flex flex-col md:flex-row py-2 gap-2" onSubmit={handleAddTask}>
        <input
          type="text"
          ...
          className="placeholder:text-center outline-0 border-4 flex-1 border-green-600 rounded-lg min-h-10 md:min-h-11 lg:h-12 px-4"
        />
        <div className=" flex justify-between  gap-2">
          <select
            value={inputTask.type}
            onChange={(e) =>
              setInputTask({ ...inputTask, type: e.target.value })
            }
            className="border-4 flex-1 border-green-600 rounded-lg h-10 md:h-11 lg:h-12 px-2 text-sm md:text-base"
          >
      ...
      <p className="min-h-5 text-sm text-red-400 w-fit mx-auto">
        {" "}
        {inputError}
      </p>
    </div>
